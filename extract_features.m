...
function [MAV_feature, VAR_feature, RMS_feature, WL_feature, ZC_feature, SSC_feature, AR_feature, CSP_feature, featureLabels] = extract_features(window_size, overlap, fs, filteredSignal, label)
    ...
    [MAV_feature, VAR_feature, RMS_feature, WL_feature, ZC_feature, SSC_feature, AR_feature, CSP_feature, featureLabels] = deal(zeros(1,len));
    ...
    for i = 1:len
        ...
        [ar_coeffs, err] = arburg(segment,4)
        % Compute the residuals of the AR model for the entire segment
        residuals = filter([1 -ar_coeffs(2:end)], 1, segment);
        AR_feature(i) = 
        %calculate_autoregressive_feature(X, ar_coeffs); % only frequency domain
